...
| guides/G-001_environment_pitfalls.md | guide | active | 环境坑,Windows,沙箱,Electron | 实战环境坑合集（Electron GUI 沙箱/rm 拦截/Git Bash set/裸变量/PowerShell 路径/pip安装冲突） | 2026-08-12 |
...
| guides/G-003_agent_channel_fallback.md | guide | active | 编排,子Agent,通道故障,出口,CDP,实例占用 | 子 Agent 通道失效→rule 46 直执行出口；CDP 前探测实例占用与 DB 备份（2026-08-16 番茄钟实战） | 2026-08-16 |
...
- `framework-skills/`：可调用能力（当前：experience-distiller 经验蒸馏）
- `guides/`：可选操作手册（当前：G-001 环境坑 · G-002 效果类验收 · G-003 子Agent通道 · G-004 monorepo构建 · G-005 web自动化 · G-006 框架合规检查）
